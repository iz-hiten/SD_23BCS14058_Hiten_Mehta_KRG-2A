...
const SidebarInner: React.FC<{ isCollapsed: boolean; onMobileClose?: () => void }> = ({ isCollapsed, onMobileClose }) => {
  const navigate = useNavigate();

  const handleCreate = () => {
    // Navigate to dashboard, then fire event so Dashboard opens its create dropdown
    navigate('/');
    // Small delay so Dashboard has mounted before receiving the event
    setTimeout(() => {
    ...
    <div className="px-4 mb-6 mt-2">
      <button
        ...
        className={cn(
          ...
          isCollapsed ? "w-12 h-12 rounded-full mx-auto hover:bg-blue-50" : "w-full py-2.5 rounded-full hover:bg-blue-50"
        )}
      >
    ...
    <nav className="flex-1 overflow-y-auto px-2 space-y-0.5">
      {navItems.map((item) => (
        ...
        >
          ...
          <span className={cn("font-semibold", isCollapsed ? "text-[10px] mt-1" : "text-[15px]")}>
            {item.name}
          </span>
        </NavLink>
      ))}
      ...
      <div className="pt-4 mt-4 border-t border-gray-100">
        ...
        {bottomItems.map((item) => (
          <NavLink
            ...
            to={item.path}
            onClick={onMobileClose}
            {...(item.name === 'Meetings' ? { 'data-tour': 'nav-meetings' } : {})}
            {...(item.name === 'Availability' ? { 'data-tour': 'nav-availability' } : {})}
            className={cn(
        ...
        <button className={cn(
          ...
          <div className={cn("flex items-center", isCollapsed ? "flex-col" : "gap-3")}>
            ...
            <span className={cn("font-semibold", isCollapsed ? "text-[10px] mt-1" : "text-[15px]")}>Help</span>
          </div>
  ...
  return (
    <>
      {/* ── Desktop sidebar (md+) ── */}
      <aside className={cn(
        "hidden md:flex bg-white border-r border-gray-200 flex-col h-screen sticky top-0 z-20 transition-all duration-300",
        ...
        <div className={cn("p-4 flex items-center mb-2", isCollapsed ? "justify-center" : "justify-between")}>
          ...
          <button
            onClick={onToggle}
            className={cn(
              "text-gray-400 hover:text-gray-600 p-1.5 rounded-full border border-gray-200 shadow-sm bg-white transition-all hover:bg-gray-50",
              isCollapsed ? "absolute -right-4 top-4 z-30" : ""
            )}
      ...
      {isMobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-[260px] bg-white flex flex-col h-full z-50 shadow-2xl">
            <div className="p-4 flex items-center justify-between mb-2">
              <div className="flex items-center gap-1">
                ...
                <span className="text-[22px] font-bold text-[#006BFF] tracking-tight ml-1">Calendly</span>
              </div>
              <button onClick={onMobileClose} className="p-1.5 text-gray-400 hover:text-gray-600 rounded-full hover:bg-gray-100">
                <X className="w-5 h-5" />
              </button>
            </div>
            <SidebarInner isCollapsed={false} onMobileClose={onMobileClose} />
          </aside>
        </div>
      )}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-gray-200 flex items-center justify-around px-2 py-1">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                ...
                isActive ? "text-[#006BFF]" : "text-gray-500"
              )
            }
          >
        ...
        <button
          onClick={onMobileClose === undefined ? undefined : () => {
            const event = new CustomEvent('open-mobile-nav');
            window.dispatchEvent(event);
          }}
          className="flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl text-gray-500"
        >
