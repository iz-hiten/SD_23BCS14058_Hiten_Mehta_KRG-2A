...
const EventTypeItem: React.FC<EventTypeItemProps> = ({ 
  ...
  return (
    ...
    >
      <div className={cn("w-[6px] shrink-0", type.color || "bg-[#006BFF]")} />
      <div className="flex-1 p-4 md:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 min-w-0">
        {/* Left: info */}
        <div className="flex items-start gap-3 min-w-0">
          <input 
            ...
            className="mt-1.5 w-4 h-4 shrink-0 rounded border-gray-300 text-[#006BFF] focus:ring-[#006BFF] cursor-pointer" 
          />
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <h3 className="text-[16px] md:text-[18px] font-bold text-[#1a1a1a] truncate">{type.name}</h3>
              {!type.isActive && <span className="text-[10px] font-bold uppercase tracking-wider bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded shrink-0">Off</span>}
            </div>
        ...
        <div className="flex items-center gap-2 shrink-0 ml-7 sm:ml-0">
          {/* Hover quick-actions — hidden on mobile, shown on hover on desktop */}
          <div className="hidden md:flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity mr-1">
            <button onClick={() => onBook(type)} title="Book meeting" className="p-2 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors">
              <CalendarPlus className="w-5 h-5" />
            </button>
            <button title="Offer time slots" onClick={() => onOffer(type)} className="p-2 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 md:px-4 py-2 border border-gray-300 rounded-full text-[13px] md:text-[14px] font-semibold text-[#1a1a1a] hover:bg-gray-50 transition-colors whitespace-nowrap"
          >
            {copiedId === type.id ? 'Copied' : <><Link2 className="w-4 h-4" /><span className="hidden sm:inline">Copy link</span></>}
          </button>
            ...
            <AnimatePresence>
              {isMenuOpen && (
                ...
                >
                  ...
                  <button onClick={() => onClone(type)} className="w-full px-4 py-2 text-left text-[14px] text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                    <Copy className="w-4 h-4" /> Clone
                  </button>
                  <button onClick={() => onToggleActive(type.id, !type.isActive)} className="w-full px-4 py-2 text-left text-[14px] text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                    <Power className="w-4 h-4" /> Turn {type.isActive ? 'off' : 'on'}
                  </button>
                  <button onClick={() => { onEdit(type); onMenuToggle(type.id); }} className="w-full px-4 py-2 text-left text-[14px] text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                    <Settings className="w-4 h-4" /> Settings
                  </button>
                  {/* Mobile-only quick actions inside menu */}
                  <div className="md:hidden h-px bg-gray-100 my-1" />
                  <button onClick={() => { onBook(type); onMenuToggle(type.id); }} className="md:hidden w-full px-4 py-2 text-left text-[14px] text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                    <CalendarPlus className="w-4 h-4" /> Book meeting
                  </button>
                  <button onClick={() => { onOffer(type); onMenuToggle(type.id); }} className="md:hidden w-full px-4 py-2 text-left text-[14px] text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                    <MailPlus className="w-4 h-4" /> Offer time slots
                  </button>
                  <button onClick={() => { onShare(type); onMenuToggle(type.id); }} className="md:hidden w-full px-4 py-2 text-left text-[14px] text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                    <Redo2 className="w-4 h-4" /> Share availability
                  </button>
                  <div className="h-px bg-gray-100 my-1" />
                  <button onClick={() => onDelete(type.id)} className="w-full px-4 py-2 text-left text-[14px] text-red-600 hover:bg-red-50 flex items-center gap-2">
...
export const Dashboard: React.FC = () => {
  ...
  // Listen for the sidebar Create button event
  useEffect(() => {
    const handler = () => setIsModalOpen('event');
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex relative" data-tour="create-btn">
          <button 
      ...
      {/* Tabs */}
      <div className="flex gap-4 md:gap-8 border-b border-gray-200 mb-6 overflow-x-auto scrollbar-none">
        {['Event types', 'Single-use links', 'Meeting polls'].map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={cn(
              "pb-3 text-[13px] md:text-[15px] font-semibold transition-all relative whitespace-nowrap shrink-0",
              activeTab === tab 
      ...
      <div className="flex items-center justify-between mb-4 px-2 gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-6 h-6 bg-[#f2f2f2] rounded-full flex items-center justify-center text-[10px] font-bold text-gray-600 shrink-0">
            {currentUser.displayName?.[0] || 'H'}
          </div>
          <span className="text-[14px] md:text-[15px] font-semibold text-[#1a1a1a] truncate">{currentUser.displayName}</span>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button 
            onClick={() => window.open(`${window.location.origin}/u/${currentUser.uid}`, '_blank')}
            className="flex items-center gap-1 text-[13px] md:text-[14px] font-semibold text-[#006BFF] hover:underline"
          >
        ...
        <div className="space-y-4 pb-24">
          ...
          {activeTab === 'Single-use links' && singleUseLinks.filter(l => l.name.toLowerCase().includes(searchQuery.toLowerCase())).map((link) => (
            <motion.div
              ...
              className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 md:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
            >
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <h3 className="text-[15px] md:text-[16px] font-bold text-[#1a1a1a] truncate">{link.name}</h3>
                  <p className="text-[13px] md:text-[14px] text-gray-500">
              ...
              <div className="flex items-center gap-2 ml-14 sm:ml-0">
                <button
                  ...
                  className="px-3 md:px-4 py-2 border border-gray-300 rounded-full text-[13px] md:text-[14px] font-semibold text-[#1a1a1a] hover:bg-gray-50"
                >
          ...
          {activeTab === 'Meeting polls' && meetingPolls.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).map((poll) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <h3 className="text-[15px] md:text-[16px] font-bold text-[#1a1a1a] truncate">{poll.name}</h3>
                  <p className="text-[13px] md:text-[14px] text-gray-500">
