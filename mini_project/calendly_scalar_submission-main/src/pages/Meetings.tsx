  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <header className="mb-8">
        ...
        {/* Tabs */}
        <div className="flex gap-4 md:gap-8 border-b border-gray-200 overflow-x-auto scrollbar-none">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-6 md:mb-8">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 text-[13px] md:text-[14px] text-gray-500">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 text-[13px] md:text-[14px] font-bold text-[#006BFF] hover:underline"
            >
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1 md:flex-none">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-[14px] w-full md:w-64"
            />
          </div>
          <button onClick={handleExport} className="flex items-center gap-1.5 px-3 md:px-4 py-2 border border-gray-300 rounded-lg text-[13px] md:text-[14px] font-bold text-[#1a1a1a] hover:bg-gray-50 whitespace-nowrap">
            Export
            ...
            return (
              <div key={meeting.id} className="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                <div className="flex flex-col sm:flex-row">
                  <div className="sm:w-[200px] md:w-[240px] p-4 border-b sm:border-b-0 sm:border-r border-gray-100 bg-gray-50/50">
                    <div className="flex items-center gap-2 mb-1">
                      <div className={cn("w-3 h-3 rounded-full shrink-0", eventType?.color || "bg-[#006BFF]")} />
                      <span className="text-[13px] md:text-[14px] font-bold text-[#1a1a1a]">{meeting.startTime} - {meeting.endTime}</span>
                    </div>
                  ...
                  <div className="flex-1 p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                      <p className="text-[14px] md:text-[15px] font-bold text-[#1a1a1a] mb-0.5">{meeting.inviteeName}</p>
                      <p className="text-[12px] md:text-[13px] text-gray-500 font-medium">Event type: <span className="text-[#1a1a1a]">{eventType?.name || 'Meeting'}</span></p>
                    </div>

                    <div className="flex items-center gap-3">
                      <button
                        onClick={() => setExpandedId(expandedId === meeting.id ? null : meeting.id)}
                        className="flex items-center gap-1.5 text-[13px] md:text-[14px] font-bold text-[#006BFF] hover:underline"
                      >
                      ...
                      {meeting.status !== 'cancelled' && activeTab === 'Upcoming' && (
                        <button
                          ...
                          className="flex items-center gap-1.5 text-[13px] md:text-[14px] font-bold text-red-500 hover:text-red-700 hover:underline transition-colors"
                        >
