...
export const AvailabilityPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Tabs */}
      <div className="flex gap-6 md:gap-8 border-b border-gray-200 mb-6 md:mb-8">
                    ...
                    {!dayData.enabled ? (
                      ...
                      <div className="space-y-3">
                        {dayData.slots.map((slot, index) => (
                          <div key={index} className="flex items-center gap-2 group flex-wrap">
                            <input
                              type="time"
                              value={slot.start}
                              onChange={(e) => updateSlot(day, index, 'start', e.target.value)}
                              className="px-2 md:px-3 py-2 border border-gray-300 rounded-lg text-[13px] md:text-[14px] focus:ring-2 focus:ring-blue-500 outline-none w-28 md:w-32"
                            />
